...
PSOut PS_MAIN(GS_OUT input)
{
    ...
    float2 noise = Unity_SimpleNoise_float(input.uv + noiseSpeed * time, 25.f);
    float2 lerpf = lerp(input.uv, noise, 0.2f);

    float rect = Unity_Rectangle_float(lerpf, 0.03f, 1.f);

    PSOut Out;
    Out.Color = rect * color;
    Out.Emission = rect * Emission;
