...
PSOut PS_MAIN(VSOut In)
{
    ...
    float noise = Unity_SimpleNoise_float(In.UV, 40);
    float diff = saturate((g_float_0 - (In.WorldPos.y + 16)) / 32 * -1);
    ...
    float4 em = (float4)0.f;

    if (diff + g_float_1 > noise)
        em = In.Emission;
