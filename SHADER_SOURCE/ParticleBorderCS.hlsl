    ...
    if (g_particle[GroupIndex].alive == 0)
    {
        while (true)
        {
            ...
            if (textureAttach > 0)
            {
                ...
                float texValue = tex_0.SampleLevel(pointSampler, uv, 0).w;
                float up = tex_0.SampleLevel(pointSampler,   uv + float2(0, -1.f / texSize.y), 0).w;
                float down = tex_0.SampleLevel(pointSampler, uv + float2(0, 1.f / texSize.y), 0).w;
                float left = tex_0.SampleLevel(pointSampler, uv + float2(-1.f / texSize.x, 0), 0).w;
                float right = tex_0.SampleLevel(pointSampler,uv + float2(1.f / texSize.x, 0), 0).w;

                if ((texValue != 0 && uv.x > 0 && uv.y > 0 && uv.x < 1 && uv.y < 1) || (up == 0 && down == 0 && left == 0 && right == 0 ))
                    break;
            }
