...
PSOut PS_MAIN(VSOut In)
{
    ...
    float3 Tint = In.Color.xyz * In.Color.w;
    if (In.Color.w > 0.01 || (Out.Diffuse.x != 1 || Out.Diffuse.z != 1)) {
        Out.Diffuse.xyz += Tint;       
        In.Emission.xyz *= In.Emission.w;
        Out.Emission = In.Emission;
    }
    else
    {
        Out.Diffuse = float4(1, 89.f / 255.f, 66.f / 255.f, 1);
        Out.Position = In.WorldPos;
        Out.LightMap = Light;
        In.Emission = 
        Out.Emission = float4(1, 89.f / 255.f, 66.f / 255.f, 1);
    }
